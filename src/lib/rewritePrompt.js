export function buildFixFlawsPrompt(mcq, feedback) {
  return `You are an expert at rewriting multiple-choice questions to correct flaws in the item.
...
export function buildBloomsPrompt(mcq, numLevels) {
  return `You are an expert at rewriting multiple-choice questions to increase the difficulty of the question.

Given a multiple-choice question, your goal is to make the question more difficult by targeting a higher level of cognitive skill according to Bloom's Taxonomy. You should increase the difficulty by ${numLevels} level(s) of Bloom's Taxonomy.
...
- The rewritten multiple-choice question should have a clear question stem, an unambiguous correct answer, and plausible but ultimately incorrect distractor choices.
- Generate each choice directly, do not precede them with a letter, number, or symbol.
- Only change the question stem, the choices, and the answer to make the question more or less difficult to answer.
- Do not add or remove any choices.
- Change the skill level of the question by exactly ${numLevels} level(s) of Bloom's Taxonomy. If numLevels is negative, decrease the difficulty by that many levels.
- The domain and concepts being tested in the multiple-choice question should be the same as the original question.
- Change difficulty specifically by requiring different cognitive skills according to Bloom's Taxonomy, not by making wording more confusing.
- In your explanation, state the level of Bloom's Taxonomy that the original question is targeting and how you changed this level in the rewritten question.
</rewriting guidelines>
...
export function buildAddDistractorsPrompt(mcq, numDistractors) {
  return `You are an expert at rewriting multiple-choice questions to add distractors to the question.

Given a multiple-choice question, your goal is to rewrite the multiple-choice question to add ${numDistractors} new distractor(s) to the question, targeting common misconceptions and making the question more difficult to answer.
...
- The rewritten multiple-choice question should have a clear question stem, an unambiguous correct answer, and plausible but ultimately incorrect distractor choices.
- Generate each choice directly, do not precede them with a letter, number, or symbol.
- Return the original distractors plus exactly ${numDistractors} new distractor(s).
